...
if ($tool -eq 'Bash') {
    ...
    $denied = @(
        ...
        @{ rx = '\bgit\s+(checkout|switch)\b';     why = 'changing branch would corrupt the worktree state' },
        @{ rx = '\bgit\s+worktree\b';              why = 'worktrees are managed by the orchestrator' },
...
if ($tool -in @('Edit','Write','MultiEdit','NotebookEdit')) {
    ...
    $blockedPaths = @(
        ...
        '.pem',
        '.pfx',
        '/scripts/ralph/' # the agent must never edit the loop's own tooling
    )
