...
.DESCRIPTION
    * One branch per RUN: the runner creates `afk/run-<stamp>` from origin/main
      in a throwaway worktree, then works the AFK queue in order, committing
      every issue onto that same branch. At the end you have ONE branch to
      review and merge into main by hand. The runner never pushes and never
      opens a PR.
...
function Invoke-Issue {
    ...
    $commits = (git -C $Wt rev-list --count "$before..$after").Trim()
    Log "  execution ended: $status ($commits commit(s) this issue)"
    return $status
}

# --- Main ---------------------------------------------------------------------
Log "Ralph run $RunStamp | deadline=$($Deadline.ToString('HH:mm')) perIssue=${MaxMinutesPerIssue}min plan=$PlanModel/$PlanEffort exec=$(if($ExecModel){$ExecModel}else{'auto'})/$ExecEffort$(if($HeadlessExec){' [headless]'}else{' [interactive]'})$(if($DryRun){' [DryRun]'})"
...
# One branch per run, in a throwaway worktree off origin/main.
...
Log "Run branch: $Branch  (worktree: $Wt)"
...
try {
    ...
    $commits = (git -C $Wt rev-list --count "origin/main..$Branch" 2>$null)
    $commits = if ($commits) { $commits.Trim() } else { '0' }
    Log "----"
    Log "Branch '$Branch' carries $commits commit(s) over origin/main."
    if ([int]$commits -gt 0) { (git -C $Wt log --oneline "origin/main..$Branch") | ForEach-Object { Log "    $_" } }
    ...
    Log "Review and merge by hand from your main checkout:  git merge $Branch"
    Log "Logs: $RunDir"
}
